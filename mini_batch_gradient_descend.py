...
batch_size = 128 #Her mini adım atmadan önce sadece bu kadar veri noktasına bakacağımızı belirliyoruz
...
for current_index in range(epochs):
    ...
    for i in range(0,dataset_len,batch_size): #start-stop-size
        ...
        for dataset_index in batch_indices:
            x_i = x[dataset_index] #Karıştırdığımız ve 128 tane ayırdığımız o x'değerlerini sıra sıra alır ve y'ler ona göre hesaplanır.
            y_i = y[dataset_index]
        ...
        w2 = w2 - (learning_rate * average_grad_w2) #Tekrar başa döner ve bi sonraki karışık olan 128'liği alarak aynı işlemleri tekrarlar.

    total_square_error = 0.0

    for dataset_index in range(dataset_len): #Hata hesaplamaları yapılır ve cost olarak diziye yazılır.
        x_i = x[dataset_index]
        y_i = y[dataset_index]
...
#1-Öncelikle veri setimiz ilk adımını atar ve tamamen karıştırılır.

#2-Sonra başka bir döngü ile küçük adımlar atmaya başlanır mesela: 1 küçük adım attımve ilk
# 128(batch_size) noktayı aldım, gradyanlarını aldım, ortalamasını aldım; 2'yi attım sonraki 
# 128'i aldım diye tüm o karışık diziyi 7812 adımda parça parça şeklinde aldık ve hepsinin 
# tek tek gradyan değerlerini hesaplayıp ortalamalarını aldık. Hepsinin ortalama gradyan değeri
# hesaplandıktan sonra ilk adımı tamamlamış olduk ve costu hesapladık. Bunu adım sayımız(epoch) 
# kadar hesaplarız.

#3-Tüm değerleri artık gösterip grafiği çizdirdik.